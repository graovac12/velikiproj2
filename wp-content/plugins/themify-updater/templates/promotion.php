...
<script>
	...
	function themify_updater_install (e , slug, product_type, nonce, action, version,title ) {
		...
		if ( typeof version === 'undefined' || version.toLowerCase().indexOf('latest') > -1){
			version = '';
		}
		else if(version && typeof themify_vars!=='undefined'){
			const vv=parseInt(version[0].trim()),
                currentV=parseInt(themify_vars.version[0].trim());
		...
		if ( typeof action !== 'undefined' && action === 'upgrade') {
			url = adminLink + "?action=upgrade-" + product_type + "&" + product_type + "=" + slug + "&_wpnonce=" + nonce + "&themify_theme_downgrade=1&version=" + version;
		} else {
	...
	function themify_updater_previous_reinstall (e, slug, product_type, nonce) {
		const title = e.target.parentNode.dataset.title + ' ' + e.target.innerHTML.toLowerCase().replace(' (latest)','');
            themify_updater_install(null, slug, product_type, nonce, 'upgrade',  e.target.innerHTML,title);
        }
